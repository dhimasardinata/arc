...
elseif(ARC_TARGET STREQUAL "esp32s31")
    ...
    set(_arc_s31_missing "")
    foreach(_arc_s31_path
            components/soc/esp32s31
            components/hal/esp32s31
            components/esp_rom/esp32s31
            components/esp_system/ld/esp32s31
            tools/cmake/toolchain-esp32s31.cmake)
        if(NOT EXISTS "$ENV{IDF_PATH}/${_arc_s31_path}")
            list(APPEND _arc_s31_missing "${_arc_s31_path}")
        endif()
    endforeach()
    if(EXISTS "$ENV{IDF_PATH}/tools/idf_py_actions/constants.py")
        file(READ "$ENV{IDF_PATH}/tools/idf_py_actions/constants.py" _arc_s31_registry)
    else()
        set(_arc_s31_registry "")
    endif()
    if(NOT _arc_s31_registry MATCHES "['\"]esp32s31['\"]")
        list(APPEND _arc_s31_missing "tools/idf_py_actions/constants.py target registration")
    endif()
    if(_arc_s31_missing)
        string(REPLACE ";" ", " _arc_s31_missing_text "${_arc_s31_missing}")
        message(FATAL_ERROR "ESP-IDF at '$ENV{IDF_PATH}' does not include complete esp32s31 target metadata. Missing: ${_arc_s31_missing_text}. Set ARC_IDF_PATH/IDF_PATH to a preview ESP-IDF that supports ESP32-S31.")
    endif()
