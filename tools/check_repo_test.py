...
class CheckRepoToolTest(unittest.TestCase):
    ...
    def test_ci_guards_cover_strict_audit_and_header_validation(self) -> None:
        ...
        self.assertIn("evidence bundle check tool must stay executable", text)
        self.assertIn("build workflow must verify repository evidence bundle coherence", text)
        self.assertIn("./tools/evidence-bundle-check.py .arc-artifacts", text)
        ...
        self.assertIn("license policy evidence check failed", text)
        self.assertIn("license policy evidence tool must stay executable", text)
        self.assertIn("build workflow must emit license policy evidence", text)
        self.assertIn("build workflow must index license policy evidence", text)
        self.assertIn("secret scan evidence check failed", text)
        self.assertIn("secret scan evidence tool must stay executable", text)
        self.assertIn("build workflow must emit secret scan evidence", text)
        self.assertIn("SBOM generation failed", text)
        self.assertIn("SBOM tool must stay executable", text)
        self.assertIn("build workflow must emit SPDX SBOM evidence", text)
        self.assertIn("build workflow must index SPDX SBOM evidence", text)
        self.assertIn("grep -qF -- '--require .arc-artifacts/sbom.spdx.json'", text)
        self.assertIn("provenance generation failed", text)
        self.assertIn("provenance tool must stay executable", text)
        self.assertIn("build workflow must emit in-toto provenance evidence", text)
        self.assertIn("build workflow must index in-toto provenance evidence", text)
        self.assertIn("grep -qF -- '--require .arc-artifacts/provenance.intoto.json'", text)
        ...
        self.assertIn("build workflow must publish repository evidence index", text)
