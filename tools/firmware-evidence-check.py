...
def parse_metadata_time(value: Any, field: str, problems: list[str]) -> datetime | None:
    ...
    try:
        ...
        problems.append(f"{PROVENANCE_NAME}: metadata {field} must be ISO-8601")
        return None
    if stamp.tzinfo is None or stamp.utcoffset() is None:
        problems.append(f"{PROVENANCE_NAME}: metadata {field} must include timezone")
        return None
    return stamp
...
def check_provenance_predicate(provenance: dict[str, Any], problems: list[str]) -> None:
    ...
    if not isinstance(dependencies, list):
        problems.append(f"{PROVENANCE_NAME}: resolvedDependencies must be a list")
    else:
        source_dependencies = [
        ...
        if len(dependencies) != 1 or len(source_dependencies) != 1:
            problems.append(f"{PROVENANCE_NAME}: resolvedDependencies must contain only arc-source")
    ...
    if not isinstance(metadata.get("invocationId"), str) or not metadata["invocationId"]:
        problems.append(f"{PROVENANCE_NAME}: metadata invocationId must be present")
    ...
    if started is not None and finished is not None and finished < started:
        problems.append(f"{PROVENANCE_NAME}: metadata finishedOn must not be before startedOn")
