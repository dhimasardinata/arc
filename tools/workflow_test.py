...
class WorkflowTest(unittest.TestCase):
    ...
    def test_repository_evidence_bundle_is_uploaded(self) -> None:
        ...
        self.assertIn(
            "./tools/evidence-workflow-check.py --format json > .arc-artifacts/evidence-workflow.json", workflow
        )
        self.assertIn("--require .arc-artifacts/evidence-workflow.json", workflow)
        self.assertIn(".arc-artifacts/evidence-workflow.json", workflow)
        self.assertIn(
        ...
        self.assertIn("--require .arc-artifacts/license-policy.json", workflow)
        self.assertIn(".arc-artifacts/license-policy.json", workflow)
    ...
    def test_changed_project_plan_gates_firmware_builds(self) -> None:
        ...
        self.assertIn("include_experimental:", workflow)
        self.assertIn("idf_ref:", workflow)
        self.assertIn("ARC_IDF_EFFECTIVE_REF", workflow)
        self.assertIn("${{ github.event.inputs.idf_ref || 'd59a48530db46665ea70e2876738062ee0a9cdc2' }}", workflow)
        self.assertIn("ARC_INCLUDE_EXPERIMENTAL", workflow)
        self.assertIn("ARC_IDF_TARGET_SET", workflow)
        self.assertIn("ARC_IDF_INSTALL_TARGETS", workflow)
        self.assertIn("esp32s3 esp32s31", workflow)
        self.assertIn("plan_args=(--buildable)", workflow)
        self.assertIn("plan_args+=(--include-experimental)", workflow)
        self.assertIn('./tools/ci-build-plan.py "${plan_args[@]}"', workflow)
        self.assertIn('target_set_marker="$HOME/.espressif/arc-idf-target-set-${ARC_IDF_TARGET_SET}"', workflow)
        self.assertIn('git -C "$HOME/esp-idf" fetch --depth 1 origin "$ARC_IDF_EFFECTIVE_REF"', workflow)
        self.assertIn('git -C "$HOME/esp-idf" checkout --force FETCH_HEAD', workflow)
        self.assertIn("could not fetch requested ESP-IDF ref: $ARC_IDF_EFFECTIVE_REF", workflow)
        self.assertIn('./tools/s31-readiness.py --idf-path "$HOME/esp-idf" --require-sdk --format report', workflow)
    ...
    def test_firmware_artifact_manifest_is_uploaded_with_binaries(self) -> None:
        ...
        self.assertIn("./tools/firmware-evidence-check.py .arc-artifacts", workflow)
        self.assertIn("if-no-files-found: error", workflow)
        self.assertIn("retention-days: 30", workflow)
