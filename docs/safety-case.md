...
| Core-local state can carry its AMP owner and scoped callback leakage checks in the C++ type. | `arc::CoreLocal<T, Core>` exposes copy-out `snapshot()`, owner-inferred `set(value)` / `with(fn)` / `msg<To>()` / `accept(msg)` helpers, `CoreLocal::Msg<To>` / `Incoming<From>` message contracts, public `CoreLocalType` / `CoreMsgType` concepts, and explicit scoped `with<Core>(fn)` callbacks only to the owner core template argument, rejects reference/pointer/reference-wrapper/non-owning-view results and standard/result wrappers containing them, and `arc::CoreMsg<T, From, To>` exposes payload snapshots and scoped destination reads with the same callback-return checks; both `CoreLocal` and `CoreMsg` reject direct borrowed-storage payload carriers, with compile-contract checks in `tests/host/logic.cpp` and negative cases in `tools/compile-fail-check.py`. |
| Static application state can carry static lifetime, access mode, optional core owner, same-contract alias checks, and scoped callback leakage checks in the C++ type. | `components/arc/include/arc/borrow.hpp` implements `arc::StaticRef`, `arc::StaticLoan`, copy-out `snapshot()` / `snapshots()` helpers, whole-value `set(value)`, `arc::StaticReads`, `arc::StaticWrites`, `arc::StaticEdit`, `arc::LoanPack`, and scoped `with_read`/`with_reads`/`with_write`/`with_edit` helpers over static objects, including `StaticRef::Reads<...>` / `StaticRef::Edit<...>` contracts plus member `StaticRef::snapshots<ReadRefs...>()`, `StaticRef::with_reads<ReadRefs...>(fn)`, and `StaticRef::with_edit<ReadRefs...>(fn)` for owner-anchored scopes, with callback checks rejecting reference/pointer/reference-wrapper/non-owning-view/static-loan returns and standard/result wrappers containing them, compile-contract checks in `tests/host/logic.cpp`, `tests/host/safety_patterns_compile.cpp`, and negative compile cases in `tools/compile-fail-check.py`. |
| Queue, fan-in, RPC, and flow payloads reject direct borrowed-storage carriers at the type boundary. | `components/arc/include/arc/spsc.hpp`, `components/arc/include/arc/mpsc.hpp`, `components/arc/include/arc/fanin.hpp`, `components/arc/include/arc/rpc.hpp`, and `components/arc/include/arc/flow.hpp` require copied payload values and reject pointer/reference-wrapper/non-owning-view payloads plus standard/result wrappers containing them, with negative compile cases in `tools/compile-fail-check.py`. |
...
| HIL evidence has a machine-checkable artifact shape. | `docs/hil-test-jig.md` defines the required physical cases, and `tools/hil-evidence-check.py --format json` validates captured JSONL artifacts before they are used as board evidence. |
