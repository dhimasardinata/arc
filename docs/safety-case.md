...
| Core-local state can carry its AMP owner and scoped callback leakage checks in the C++ type. | `arc::CoreLocal<T, Core>` exposes copy-out `snapshot()`, owner-inferred `with(fn)` / `msg<To>()` / `accept(msg)` helpers, `CoreLocal::Msg<To>` / `Incoming<From>` message contracts, public `CoreLocalType` / `CoreMsgType` concepts, and explicit scoped `with<Core>(fn)` callbacks only to the owner core template argument, rejects reference/pointer callback results, and `arc::CoreMsg<T, From, To>` exposes payload snapshots and scoped destination reads with the same callback-return checks, with compile-contract checks in `tests/host/logic.cpp` and negative cases in `tools/compile-fail-check.py`. |
| Static application state can carry static lifetime, access mode, optional core owner, same-contract alias checks, and scoped callback leakage checks in the C++ type. | `components/arc/include/arc/borrow.hpp` implements `arc::StaticRef`, `arc::StaticLoan`, copy-out `snapshot()` / `snapshots()` helpers, `arc::StaticReads`, `arc::StaticWrites`, `arc::StaticEdit`, `arc::LoanPack`, and scoped `with_read`/`with_reads`/`with_write`/`with_edit` helpers over static objects, including `StaticRef::Reads<...>` / `StaticRef::Edit<...>` contracts plus member `StaticRef::snapshots<ReadRefs...>()`, `StaticRef::with_reads<ReadRefs...>(fn)`, and `StaticRef::with_edit<ReadRefs...>(fn)` for owner-anchored scopes, with compile-contract checks in `tests/host/logic.cpp`, `tests/host/safety_patterns_compile.cpp`, and negative compile cases in `tools/compile-fail-check.py`. |
...
| Queue, fan-in, and RPC direct mutation can be removed from the static type surface. | `arc::Roles<Lane>` owns a lane privately, exposes only role endpoints such as producer/consumer or client/server, and offers scoped role callbacks that reject endpoint/reference/pointer returns, with compile-contract checks and host coverage in `tests/host/logic.cpp`, `tests/host/safety_patterns_compile.cpp`, and `tools/compile-fail-check.py`. |
...
| Workloads can carry source-visible proof metadata beside a measured cycle budget. | `components/arc/include/arc/proof.hpp` implements `arc::proof::Pack` and `arc::proof::Fact` compile-time claim arrays, with host coverage in `tests/host/logic.cpp`. |
