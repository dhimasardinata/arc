...
| Core-local state can carry its AMP owner and scoped callback leakage checks in the C++ type. | `arc::CoreLocal<T, Core>` exposes local reads/writes and scoped `with<Core>(fn)` callbacks only to the owner core template argument, rejects reference/pointer callback results, and `arc::CoreMsg<T, From, To>` only exposes payload reads to the destination core, with compile-contract checks in `tests/host/logic.cpp` and negative cases in `tools/compile-fail-check.py`. |
