...
| Static application state can carry static lifetime, access mode, optional core owner, same-contract alias checks, and scoped callback leakage checks in the C++ type. | `components/arc/include/arc/borrow.hpp` implements `arc::StaticRef`, `arc::StaticLoan`, copy-out `snapshot()` / `snapshots()` helpers, `arc::StaticReads`, `arc::StaticWrites`, `arc::StaticEdit`, `arc::LoanPack`, and scoped `with_read`/`with_reads`/`with_write`/`with_edit` helpers over static objects, including member `StaticRef::snapshots<ReadRefs...>()`, `StaticRef::with_reads<ReadRefs...>(fn)`, and `StaticRef::with_edit<ReadRefs...>(fn)` for owner-anchored scopes, with compile-contract checks in `tests/host/logic.cpp`, `tests/host/safety_patterns_compile.cpp`, and negative compile cases in `tools/compile-fail-check.py`. |
