...
- Set `ARC_TARGET` first when a build needs a non-default target, then run `source ./env.sh`.
- Use the default `esp32s3` target unless a script or CI matrix needs to be explicit.
