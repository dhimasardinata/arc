...
CRC sealing and background scan state for fixed memory regions.
...
- Header: `arc/scrub.hpp`
- Module group: Memory, DMA, And Placement
- CMake feature: `scrub`
- Closest example: `examples/esp32s3/copy`

Declare `arc_requires(main_requires core scrub)` in the component that includes this header.
...
arc_requires(main_requires core scrub)
...
Source landmarks: `ScrubRegion`, `ScrubFault`, `ScrubSample`, `Scrub`.
