...
Flash, partition, image, and heap capacity reporting.

## Fit

- Use it when flash, files, NVS, OTA, or capacity checks must stay recoverable and Core 0-owned.
- Do not start here when a realtime loop would wait on flash, VFS, or partition work.
- Verification focus: record partition assumptions, rollback behavior, and the exact failure path before shipping update logic.

## Arc Contract

- Header: `arc/space.hpp`
- Module group: Storage And Update
- CMake feature: `space`
- Closest example: `examples/esp32s3/space`

Declare `arc_requires(main_requires core space)` in the component that includes this header.
...
arc_requires(main_requires core space)
...
Source landmarks: `Space`, `Percent`, `AppArea`.
...
The closest shipped example is `examples/esp32s3/space`.

```sh
. ./env.sh
idf.py -C examples/esp32s3/space build
idf.py -C examples/esp32s3/space -p /dev/ttyACM0 flash monitor
```
