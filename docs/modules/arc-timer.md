...
GPTimer timebase and alarm hooks.

## What It Owns

- Header: `arc/timer.hpp`
- Module group: GPIO, Timing, And Power
- CMake feature: `timer`
- Closest example: `examples/esp32s3/timer`

Declare the CMake feature with `arc_requires(main_requires core timer)` when this header needs ESP-IDF components beyond the base Arc component.
...
The closest shipped example is `examples/esp32s3/timer`.

```sh
. ./env.sh
idf.py -C examples/esp32s3/timer build
idf.py -C examples/esp32s3/timer -p /dev/ttyACM0 flash monitor
```
...
```text
I (000) arc-doc: module=arc/timer
I (001) arc-doc: feature=timer example=timer
