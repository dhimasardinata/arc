...
- CMake feature: `temp`
- Closest example: `examples/esp32s3/temp`

Declare the CMake feature with `arc_requires(main_requires core temp)` when this header needs ESP-IDF components beyond the base Arc component.
...
arc_requires(main_requires core temp)
...
The closest shipped example is `examples/esp32s3/temp`.

```sh
. ./env.sh
idf.py -C examples/esp32s3/temp build
idf.py -C examples/esp32s3/temp -p /dev/ttyACM0 flash monitor
```
...
```text
I (000) arc-doc: module=arc/temp
I (001) arc-doc: feature=temp example=temp
