...
Time-aware Ethernet gate schedule checks for deterministic transmit windows.
...
Declare `arc_requires(main_requires core tsn)` in the component that includes this header.
...
arc_requires(main_requires core tsn)
...
Source landmarks: `TsnGate`, `TsnWindow`, `TsnSchedule`.
