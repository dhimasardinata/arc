...
PCNT pulse and quadrature counting.

## What It Owns

- Header: `arc/count.hpp`
- Module group: GPIO, Timing, And Power
- CMake feature: `pcnt`
- Closest example: `examples/esp32s3/count`

Declare the CMake feature with `arc_requires(main_requires core pcnt)` when this header needs ESP-IDF components beyond the base Arc component.
...
arc_requires(main_requires core pcnt)
...
The closest shipped example is `examples/esp32s3/count`.

```sh
. ./env.sh
idf.py -C examples/esp32s3/count build
idf.py -C examples/esp32s3/count -p /dev/ttyACM0 flash monitor
```
...
```text
I (000) arc-doc: module=arc/count
I (001) arc-doc: feature=pcnt example=count
