...
Reusable ESP-NOW transport plane.

## What It Owns

- Header: `arc/espnow.hpp`
- Module group: Network, Radio, And Wire Protocols
- CMake feature: `espnow`
- Closest example: `examples/esp32s3/espnow`

Declare the CMake feature with `arc_requires(main_requires core espnow)` when this header needs ESP-IDF components beyond the base Arc component.
...
arc_requires(main_requires core espnow)
...
The closest shipped example is `examples/esp32s3/espnow`.

```sh
. ./env.sh
idf.py -C examples/esp32s3/espnow build
idf.py -C examples/esp32s3/espnow -p /dev/ttyACM0 flash monitor
```
...
```text
I (000) arc-doc: module=arc/espnow
I (001) arc-doc: feature=espnow example=espnow
