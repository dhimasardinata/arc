...
RTC-memory SPSC lane for ULP/main-core handoff.

## Fit

- Use it when Core 1 or an ISR-adjacent lane needs bounded handoff without FreeRTOS queue allocation.
- Do not start here when the producer/consumer shape is not fixed yet or blocking semantics are acceptable.
- Verification focus: name the producer and consumer, size the queue from worst-case bursts, and drain logs on Core 0.

## Arc Contract

- Header: `arc/rtc_ring.hpp`
- Module group: Lock-Free Lanes
- CMake feature: `rtc`
- Closest example: `examples/esp32s3/probe`

Declare `arc_requires(main_requires core rtc)` in the component that includes this header.
...
arc_requires(main_requires core rtc)
...
```cpp
#include "arc/rtc_ring.hpp"
```

## Source Landmarks

Source landmarks: `RtcRing`.
...
The closest shipped example is `examples/esp32s3/probe`.

```sh
. ./env.sh
idf.py -C examples/esp32s3/probe build
idf.py -C examples/esp32s3/probe -p /dev/ttyACM0 flash monitor
```
