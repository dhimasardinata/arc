...
Declare the CMake feature with `arc_requires(main_requires core sha)` when this header needs ESP-IDF components beyond the base Arc component.
...
arc_requires(main_requires core sha)
...
```text
I (000) arc-doc: module=arc/sha
I (001) arc-doc: feature=sha example=store
