...
ESP-IDF HTTP/HTTPS client RAII wrapper.
...
Declare the CMake feature with `arc_requires(main_requires core http)` when this header needs ESP-IDF components beyond the base Arc component.
...
arc_requires(main_requires core http)
...
```text
I (000) arc-doc: module=arc/http
I (001) arc-doc: feature=http example=udp
