...
Complementary MCPWM pairs with dead-time and optional brake input.

## What It Owns

- Header: `arc/bridge.hpp`
- Module group: GPIO, Timing, And Power
- CMake feature: `mcpwm`
- Closest example: `examples/esp32s3/bridge`

Declare the CMake feature with `arc_requires(main_requires core mcpwm)` when this header needs ESP-IDF components beyond the base Arc component.
...
arc_requires(main_requires core mcpwm)
...
The closest shipped example is `examples/esp32s3/bridge`.

```sh
. ./env.sh
idf.py -C examples/esp32s3/bridge build
idf.py -C examples/esp32s3/bridge -p /dev/ttyACM0 flash monitor
```
...
```text
I (000) arc-doc: module=arc/bridge
I (001) arc-doc: feature=mcpwm example=bridge
