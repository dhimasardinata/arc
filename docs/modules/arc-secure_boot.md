...
Secure-boot state and policy hooks.

## What It Owns

- Header: `arc/secure_boot.hpp`
- Module group: Crypto, Security, VM, And Sandbox
- CMake feature: `secure_boot`
- Closest example: `examples/esp32s3/store`

Declare the CMake feature with `arc_requires(main_requires core secure_boot)` when this header needs ESP-IDF components beyond the base Arc component.
...
arc_requires(main_requires core secure_boot)
...
```text
I (000) arc-doc: module=arc/secure_boot
I (001) arc-doc: feature=secure_boot example=store
