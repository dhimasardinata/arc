...
Heapless `select()` wrapper for caller-owned sockets.
...
Declare the CMake feature with `arc_requires(main_requires core poll)` when this header needs ESP-IDF components beyond the base Arc component.
...
arc_requires(main_requires core poll)
...
```text
I (000) arc-doc: module=arc/poll
I (001) arc-doc: feature=poll example=udp
