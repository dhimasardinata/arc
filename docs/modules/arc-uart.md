...
UART ports, pins, framing, buffers, and runtime baud changes.
...
- CMake feature: `uart`
- Closest example: `examples/esp32s3/uart`

Declare the CMake feature with `arc_requires(main_requires core uart)` when this header needs ESP-IDF components beyond the base Arc component.
...
The closest shipped example is `examples/esp32s3/uart`.

```sh
. ./env.sh
idf.py -C examples/esp32s3/uart build
idf.py -C examples/esp32s3/uart -p /dev/ttyACM0 flash monitor
```
...
```text
I (000) arc-doc: module=arc/uart
I (001) arc-doc: feature=uart example=uart
