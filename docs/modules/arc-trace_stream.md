# `arc/trace_stream.hpp`

Draining binary logs to UDP, WebSocket, file, or custom sinks.

## Fit

- Use it when runtime evidence must leave the hot path as compact events and be formatted later.
- Do not start here when the hot loop would format strings, allocate JSON, or block on a sink.
- Verification focus: prove the producer only writes bounded records and the drain path owns formatting, files, UDP, or WebSocket output.

## Arc Contract

- Header: `arc/trace_stream.hpp`
- Module group: Observability And Trace
- CMake feature: `trace_live`
- Closest example: `examples/esp32s3/trace`

Declare `arc_requires(main_requires core trace_live)` in the component that includes this header.
...
arc_requires(main_requires core trace_live)
...
The closest shipped example is `examples/esp32s3/trace`.

```sh
. ./env.sh
idf.py -C examples/esp32s3/trace build
idf.py -C examples/esp32s3/trace -p /dev/ttyACM0 flash monitor
```
