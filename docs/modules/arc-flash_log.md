...
Fixed-record queue flushed through a storage sink.
...
- Header: `arc/flash_log.hpp`
- Module group: Storage And Update
- CMake feature: `store`
- Closest example: `examples/esp32s3/store`

Declare `arc_requires(main_requires core store)` in the component that includes this header.
...
arc_requires(main_requires core store)
