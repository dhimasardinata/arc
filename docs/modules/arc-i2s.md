...
Standard, TDM, and PDM I2S DMA lanes.
...
- CMake feature: `i2s`
- Closest example: `examples/esp32s3/i2s`

Declare the CMake feature with `arc_requires(main_requires core i2s)` when this header needs ESP-IDF components beyond the base Arc component.
...
arc_requires(main_requires core i2s)
...
The closest shipped example is `examples/esp32s3/i2s`.

```sh
. ./env.sh
idf.py -C examples/esp32s3/i2s build
idf.py -C examples/esp32s3/i2s -p /dev/ttyACM0 flash monitor
```
...
```text
I (000) arc-doc: module=arc/i2s
I (001) arc-doc: feature=i2s example=i2s
