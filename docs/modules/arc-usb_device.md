...
USB device descriptors, Chapter 9 state, class-facing FIFO bridges.

## What It Owns

- Header: `arc/usb_device.hpp`
- Module group: Buses, Displays, And Data Capture
- CMake feature: `usb_device`
- Closest example: `examples/esp32s3/spi`

Declare the CMake feature with `arc_requires(main_requires core usb_device)` when this header needs ESP-IDF components beyond the base Arc component.
...
arc_requires(main_requires core usb_device)
...
The closest shipped example is `examples/esp32s3/spi`.

```sh
. ./env.sh
idf.py -C examples/esp32s3/spi build
idf.py -C examples/esp32s3/spi -p /dev/ttyACM0 flash monitor
```
...
```text
I (000) arc-doc: module=arc/usb_device
I (001) arc-doc: feature=usb_device example=spi
