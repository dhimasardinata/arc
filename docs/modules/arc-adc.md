...
ADC pad descriptors and oneshot reads.
...
- CMake feature: `adc`
- Closest example: `examples/esp32s3/scope`

Declare the CMake feature with `arc_requires(main_requires core adc)` when this header needs ESP-IDF components beyond the base Arc component.
...
arc_requires(main_requires core adc)
...
The closest shipped example is `examples/esp32s3/scope`.

```sh
. ./env.sh
idf.py -C examples/esp32s3/scope build
idf.py -C examples/esp32s3/scope -p /dev/ttyACM0 flash monitor
```
...
```text
I (000) arc-doc: module=arc/adc
I (001) arc-doc: feature=adc example=scope
