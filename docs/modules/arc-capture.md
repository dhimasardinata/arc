...
MCPWM capture timestamps.

## What It Owns

- Header: `arc/capture.hpp`
- Module group: GPIO, Timing, And Power
- CMake feature: `mcpwm`
- Closest example: `examples/esp32s3/pulse`
...
The closest shipped example is `examples/esp32s3/pulse`.

```sh
. ./env.sh
idf.py -C examples/esp32s3/pulse build
idf.py -C examples/esp32s3/pulse -p /dev/ttyACM0 flash monitor
```
...
```text
I (000) arc-doc: module=arc/capture
I (001) arc-doc: feature=mcpwm example=pulse
