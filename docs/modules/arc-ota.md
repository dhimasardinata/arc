...
Staged OTA write, slot state, confirm, and rollback.
...
- CMake feature: `ota`
- Closest example: `examples/esp32s3/ota`

Declare `arc_requires(main_requires core ota)` in the component that includes this header.
...
arc_requires(main_requires core ota)
...
The closest shipped example is `examples/esp32s3/ota`.

```sh
. ./env.sh
idf.py -C examples/esp32s3/ota build
idf.py -C examples/esp32s3/ota -p /dev/ttyACM0 flash monitor
```
