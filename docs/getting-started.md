...
3. [Module Guide](/modules) maps every public header to the problem it solves.
4. [Module Page Index](/module-pages) gives one page per public header, including zero-start steps and proof paths.
5. [Examples](/examples) shows which firmware project to build for each hardware lane.
6. [API Reference](/api) gives the exact public methods, failure behavior, and ownership notes.
7. [Licensing](/licensing) explains the AGPL public path and paid commercial path.
8. [Benchmarking](/benchmarking) explains what evidence is required before publishing performance claims.
