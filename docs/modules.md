...
- use `arc/net_codecs.hpp` when bytes need MQTT, WebSocket, CoAP, URI, stream,
  or fixed binary record framing without a network task;
...
| `arc/net_codecs.hpp` | URI, streams, fixed records, MQTT, WebSocket, CoAP, and small HTTP server helpers without owning Wi-Fi. |
