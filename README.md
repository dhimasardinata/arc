...
[![codeql](https://github.com/dhimasardinata/arc/actions/workflows/codeql.yml/badge.svg)](https://github.com/dhimasardinata/arc/actions/workflows/codeql.yml)
...
`THIRD_PARTY_MANIFEST.json` records the dependency groups that are not
Arc-covered source. Run `./tools/third-party-manifest-check.py --format json`
before relying on `THIRD_PARTY_NOTICES.md` in a release record, and run
`./tools/sbom.py --format json` when the release record needs SPDX 2.3 SBOM
evidence for those groups plus docs npm packages. Run
`./tools/license-policy-check.py --format json` to reject missing or unapproved
docs dependency licenses before publishing a docs site or evidence bundle.
