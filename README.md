...
- `arc::StaticRef<&object, Core>` creates zero-cost `StaticLoan` read/mutable views, while `arc::LoanPack<...>` rejects static borrow packs that mix a mutable loan with any other loan to the same object.
...
- `arc::proof::Pack` carries compile-time proof facts and verified cycle budgets beside workloads, giving release tooling a small proof-carrying metadata surface without heap or RTTI.
...
- `arc::hls` marks fixed-bound kernels and provides unrolled FIR/dot helpers so DSP/ML/control code can stay compatible with HLS-style synthesis flows.
...
4. Open `docs/troubleshooting.md` when setup, CMake, editor, docs-site, benchmark, or evidence checks fail.
5. Use `docs/glossary.md` to decode Arc-specific terms.
6. Use `docs/modules.md` to choose the right Arc header.
7. Use `docs/module-pages.md` for a page dedicated to that header.
...
- `arc::StaticRef<&object, Owner>` is the compile-time form for static app state. `read<Core>()` returns a copyable readonly `StaticLoan`, `write<Core>()` returns a move-only mutable `StaticLoan`, and both disappear from the overload set for the wrong core owner or const storage. `arc::LoanPack<...>` is the task-contract form: repeated readonly loans are allowed, but a mutable static loan cannot be packed beside another loan to the same object.
