...
- `examples/bench` is the real ESP32-S3 benchmark firmware. It flashes to hardware, reports target cycle-counter measurements for Arc primitives/codecs/stream helpers/DSP/copy/silicon/system lanes, compares Arc against raw ESP-IDF silicon APIs in the same firmware image, covers internal temp/NVS/OTA plus self-test TWAI loopback in one run, and can add Arduino-ESP32 core write/base64 baselines when `arduino-esp32/` or `ARC_ARDUINO_PATH` is present during configure.
