...
- `arc::sim::Drive`, `Sense`, `Fifo`, `TraceLog`, and `Harness` give host builds a fixed simulator surface for pin output traces, virtual input samples, and deterministic SILS timelines.
...
- `arc::sim::Drive`, `Sense`, `TraceLog`, and `Harness` are the host-side pin surface when app logic needs deterministic policy traces and FIFO-fed inputs without touching ESP32 registers.
